...
const readTime = (text = "") => {
  const words = text?.split(" ").length || 200;
...
const HeroCard = ({ article }) => {
  ...
  const slug = `/${article.category?.toLowerCase()}/${article.slug}`;
  return (
    <div className="cp-hero-card">
      <Link to={slug} className="cp-hero-card-img-wrap">
      ...
      <div className="cp-hero-card-body">
        <span className="cp-cat-pill">{article.category}</span>
        <Link to={slug}>
          <h2 className="cp-hero-card-title">{article.title}</h2>
        </Link>
        <p className="cp-hero-card-summary">{article.summary}</p>
        <div className="cp-meta-row">
          <span className="cp-meta"><FaClock size={11} /> {formatDate(article.createdAt)}</span>
          <span className="cp-meta">{readTime(article.body)} min read</span>
...
const ArticleCard = ({ article }) => {
  ...
  return (
    <div className="cp-card">
      <Link to={slug} className="cp-card-img-wrap">
        <img src={img} alt={article.title} loading="lazy" />
        {article.isBreaking && <span className="cp-badge cp-badge-breaking">Breaking</span>}
      ...
      <div className="cp-card-body">
        <span className="cp-cat-pill">{article.category}</span>
        <Link to={slug}><h3 className="cp-card-title">{article.title}</h3></Link>
        <p className="cp-card-summary">{article.summary}</p>
        <div className="cp-meta-row">
...
const ListCard = ({ article, rank }) => {
  const slug = `/${article.category?.toLowerCase()}/${article.slug}`;
  return (
    <Link to={slug} className="cp-list-card">
      ...
      {article.coverImage && (
        <img src={article.coverImage} alt="" className="cp-list-thumb" />
      )}
    </Link>
  );
};
...
const SidebarCard = ({ article }) => {
  ...
  const slug = `/${article.category?.toLowerCase()}/${article.slug}`;
  return (
    <Link to={slug} className="cp-sidebar-card">
      <img src={img} alt={article.title} loading="lazy" />
...
export default function CategoryPage({ category, label, description, color, icon: Icon }) {
  ...
  useEffect(() => {
    ...
    getArticles({ status: "published", limit: 5, sort: "-views" })
      .then((r) => setTrending(r.data?.articles || []))
      .catch(() => {});
  }, [load]);
  ...
  return (
    <div className="cp-page">
      ...
      <div
        className="cp-banner"
        style={{ background: `linear-gradient(135deg, #0a1628 0%, ${color || "#0047AB"} 100%)` }}
      >
              ...
              <>
                ...
                {hasMore && (
                  <div className="cp-load-more">
                    ...
                    >
                      {loading ? (
                        <><span className="cp-spinner" /> Loading...</>
                      ) : (
                        "Load More Stories"
                      )}
                    </button>
            ...
            <div className="cp-sidebar">
              ...
              <div className="cp-sidebar-widget">
                ...
                <div className="cp-trending-list">
                  {trending.map((a, i) => (
                    <ListCard key={a._id} article={a} rank={i + 1} />
                  ))}
                </div>
              </div>
              ...
              <div className="cp-sidebar-widget">
                ...
                {articles.slice(0, 5).map((a) => (
                  <SidebarCard key={a._id} article={a} />
                ))}
              </div>

              {/* Newsletter CTA */}
              <div className="cp-newsletter-widget">
                <div className="cp-nl-icon">✉️</div>
                <h4>Stay Informed</h4>
              ...
              <div className="cp-ad-box">
                <span className="cp-ad-label">Advertisement</span>
                <div className="cp-ad-content">
                  <p className="cp-ad-title">AfriLENS Premium</p>
                  <small>Support African Journalism</small>
                  <a href="#" className="cp-ad-cta">Learn More →</a>
                </div>
              </div>
            </div>
          </div>
